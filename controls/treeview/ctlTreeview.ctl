...
Public Property Get Path() As String
    ...
    utvi.pszText = String(255, Chr(0)) & Chr(0)
    utvi.mask = TVIF_TEXT
    utvi.cchTextMax = 255
    
    SendMessage hTree, TVM_GETITEM, 0, utvi
    
    strOut = TrimZero(utvi.pszText) & IIf(strOut <> "", "\" & Trim(strOut), "")
    
    hPar = SendMessageLong(hTree, TVM_GETNEXTITEM, TVGN_PARENT, TNodes(m_Item).hItem)
    ...
    Do While Not hPar = 0
        ...
        hPar = SendMessageLong(hTree, TVM_GETNEXTITEM, TVGN_PARENT, utvi.hItem)
        
    Loop
    
    If Left(strOut, 1) = "\" Then strOut = Mid(strOut, 2)
...
Private Function TrimZero(ByVal strIn As String) As String

    On Error Resume Next
    
    Dim intTmp As Long
    
    intTmp = InStr(1, strIn, Chr(0))
    If intTmp > 0 Then
        TrimZero = Left(strIn, intTmp - 1)
    Else
        TrimZero = Trim(strIn)
    End If
